export type DesignSystem = {
  ...
  mode: "public"|"corporate"|"youth"|"tech"|"warm"|"editorial";
};
...
export function recommendDesign(institutionType="", audience="", topic="") {
  ...
  if(/청년|20대|대학생|창업/.test(text)) return designSystems.find(x=>x.key==="youth_pop")!;
  if(/공공|공기업|학교|교육기관|재단|공단|정부/.test(text)) return designSystems.find(x=>x.key==="public_data")!;
