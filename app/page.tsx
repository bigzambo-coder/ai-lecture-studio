  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        {view === "brief" && active && <Brief project={active} objectiveText={objectiveText} setObjectiveText={setObjectiveText} onChange={(brief) => patchActive((p) => ({ ...p, brief }))} onApprove={approveBrief} />}
...
function CreateProject({ draft, setDraft, selected, toggle, onNext }: { draft: MasterBrief; setDraft: (b: MasterBrief) => void; selected: DeliverableKey[]; toggle: (k: DeliverableKey) => void; onNext: () => void }) {
  ...
  return <div className="page form-page"><div className="form-heading"><span className="step-badge">01</span><div><span className="eyebrow">NEW LECTURE</span><h1>새 강의의 기본 정보를 알려주세요.</h1><p>확인된 내용은 다시 묻지 않고, 부족한 정보만 다음 단계에서 질문합니다.</p></div></div>
    <section className="form-section"><div className="section-index">A</div><div className="form-content"><h2>기관과 교육</h2><div className="form-grid"><label>기관명<input value={draft.institutionName} onChange={e=>field("institutionName",e.target.value)} placeholder="예: 부산시설공단"/></label><label>기관 유형<select value={draft.institutionType} onChange={e=>field("institutionType",e.target.value)}><option value="">선택해주세요</option><option>공공기관</option><option>공기업</option><option>일반기업</option><option>학교·교육기관</option><option>복지기관</option><option>청년·창업기관</option><option>소상공인 지원기관</option></select></label><label className="wide">교육 주제<input value={draft.topic} onChange={e=>field("topic",e.target.value)} placeholder="예: 생성형 AI를 활용한 업무효율화"/></label><label>교육 대상<input value={draft.audience} onChange={e=>field("audience",e.target.value)} placeholder="예: 신규직원, 영유아 부모"/></label><label>인원<input value={draft.audienceCount} onChange={e=>field("audienceCount",e.target.value)} placeholder="예: 25명"/></label><label>총 교육시간<input value={draft.totalMinutes} onChange={e=>field("totalMinutes",e.target.value)} placeholder="예: 180분"/></label><label>차시<input value={draft.sessions} onChange={e=>field("sessions",e.target.value)} placeholder="예: 1"/></label><label className="wide">교육 목적<textarea value={draft.purpose} onChange={e=>field("purpose",e.target.value)} placeholder="교육 후 참여자가 어떻게 달라지기를 원하는지 적어주세요."/></label><label className="wide">PPT 디자인 참고 사이트 또는 URL<input value={draft.designReferenceUrl??""} onChange={e=>field("designReferenceUrl",e.target.value)} placeholder="Canva·Behance·기관 홈페이지 등 참고 주소"/></label><label className="wide">Notion 워크북 새 페이지 URL<input value={draft.notionUrl??""} onChange={e=>field("notionUrl",e.target.value)} placeholder="편집 가능한 빈 Notion 페이지 주소"/></label></div></div></section>
    <section className="form-section"><div className="section-index">A2</div><div className="form-content"><h2>제출용 기획서 설정</h2><div className="plan-recommend"><span>AUTO CLASSIFICATION</span><b>{recommendedPlan.key} · {recommendedPlan.name}</b><p>{recommendedPlan.document} · {recommendedPlan.focus}</p></div><div className="form-grid plan-fields"><label>담당부서<input value={draft.department??""} onChange={e=>field("department",e.target.value)} placeholder="미정이면 [직접 입력]으로 출력"/></label><label>기획서 유형<select value={draft.planType??"auto"} onChange={e=>field("planType",e.target.value)}><option value="auto">자동 판별</option>{planTypes.map(t=><option key={t.key} value={t.key}>{t.key} · {t.name}</option>)}</select></label><label>교육 장소·환경<input value={draft.location??""} onChange={e=>field("location",e.target.value)} placeholder="예: 전산교육장 / 노트북 / Wi-Fi"/></label><label>예산<input value={draft.budget??""} onChange={e=>field("budget",e.target.value)} placeholder="미정이면 [직접 입력]"/></label><label className="wide">참여자 최종 결과물<input value={draft.finalDeliverable??""} onChange={e=>field("finalDeliverable",e.target.value)} placeholder="예: SNS 게시물 3개와 카드뉴스 4장"/></label></div></div></section>
    <section className="form-section"><div className="section-index">B</div><div className="form-content"><h2>PPT 디자인 템플릿 선택</h2><p className="hint">업로드한 17개 원본 레퍼런스와 전체 분석 몽타주를 함께 분석했습니다. 원본을 복사하지 않고 기관·대상에 맞는 색상, 그리드, 타이포, 도표 문법으로 재설계합니다.</p><div className="reference-analysis"><div><b>18</b><span>분석 자료</span></div><div><b>16</b><span>울트라와이드 레퍼런스</span></div><div><b>{designSystems.length}</b><span>맞춤 디자인 시스템</span></div><p>기관의 공식성 + 대상의 연령·직업 특성 + 강의 주제를 동시에 반영</p></div><div className="design-recommend"><div><span>AUTO RECOMMEND</span><b>{recommended.name}</b><p>{draft.institutionType||"기관 유형"}과 {draft.audience||"교육 대상"}에 {recommended.fit} 계열이 가장 잘 맞습니다.</p></div><button type="button" onClick={()=>field("designPreset",recommended.key)}>추천안 적용</button></div><div className="template-grid">{designSystems.map((p)=><button type="button" key={p.key} className={`template-card ${draft.designPreset===p.key?"selected":""}`} onClick={()=>field("designPreset",p.key)}><div className="reference-thumb" style={{backgroundImage:`linear-gradient(180deg,transparent 35%,rgba(0,0,0,.72)),url(${p.preview})`}}><em>REF {p.references.join(" · ")}</em></div><div><b>{p.name}</b><small>{p.description}</small><mark>{p.fit}</mark></div></button>)}</div></div></section>
    <section className="form-section"><div className="section-index">C</div><div className="form-content"><h2>필요한 결과물</h2><p className="hint">하나만 선택하거나 전체를 선택할 수 있습니다. PPT만 선택해도 필요한 내부 설계는 자동으로 진행됩니다.</p><div className="deliverable-grid">{deliverableOptions.map(o=><button key={o.key} type="button" className={selected.includes(o.key)?"deliverable selected":"deliverable"} onClick={()=>toggle(o.key)}><span>{o.icon}</span><div><b>{o.title}</b><small>{o.description}</small></div><i>{selected.includes(o.key)?"✓":"＋"}</i></button>)}</div><button type="button" className="select-all" onClick={()=>deliverableOptions.forEach(o=>{if(!selected.includes(o.key)) toggle(o.key)})}>전체 패키지 선택</button></div></section>
    <div className="form-actions"><span>입력한 내용은 자동 저장될 프로젝트의 기준정보가 됩니다.</span><button className="primary" disabled={!draft.topic || !draft.audience || selected.length===0} onClick={onNext}>추가 인터뷰 시작 <b>→</b></button></div>
  </div>;
}
...
function Brief({ project, onChange, onApprove }: { project: Project; objectiveText: string; setObjectiveText:(s:string)=>void; onChange:(b:MasterBrief)=>void; onApprove:()=>void }) {
  const b=project.brief; const change=(key:keyof MasterBrief,value:string|string[])=>onChange({...b,[key]:value});
