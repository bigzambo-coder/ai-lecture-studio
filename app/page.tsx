  ...
  useEffect(() => {
    try {
      ...
      const parsed=(raw as Project[]).map((project)=>{
        ...
        if(artifacts.length>0&&artifacts.every((artifact)=>(artifact.engineVersion??0)>=17))return {...project,brief,artifacts,stages:visibleStages,currentStage:visibleStages.some(stage=>stage.key===project.currentStage&&stage.selected)?project.currentStage:visibleCurrent};
...
function CreateProject({ draft, setDraft, selected, toggle, onNext }: { draft: MasterBrief; setDraft: (b: MasterBrief) => void; selected: DeliverableKey[]; toggle: (k: DeliverableKey) => void; onNext: () => void }) {
  ...
  return <div className="page form-page wizard-page"><div className="wizard-head"><div><span className="eyebrow">NEW LECTURE · STEP {step+1}/4</span><h1>{titles[step]}</h1><p>한 화면씩 입력하면 자동 저장됩니다.</p></div><div className="wizard-progress">{titles.map((t,i)=><button type="button" key={t} className={i===step?"active":i<step?"done":""} onClick={()=>i<=step&&setStep(i)}><span>{i<step?"✓":i+1}</span>{t}</button>)}</div></div>
    <div className="wizard-card">
      ...
      {step===2&&<><div className="design-source"><div><span>ATOMIC DESIGN SYSTEM</span><b>기관·대상·주제에 맞는 3가지 방향을 골랐습니다.</b><p>색상만 바꾸지 않고 기초 규칙, 정보 블록, 슬라이드 구조를 하나의 체계로 적용합니다.</p></div><span className="system-badge">SYSTEM BASED</span></div><div className="atomic-map" aria-label="Atomic Design 적용 구조"><div><span>01 · FOUNDATION</span><b>글꼴 · 색 · 간격</b></div><i>→</i><div><span>02 · MOLECULE</span><b>제목 · 근거 · 시각자료</b></div><i>→</i><div><span>03 · ORGANISM</span><b>표지 · 개념 · 실습 · 도표</b></div></div><div className="design-options">{recommendations.map((p,i)=><button type="button" key={p.key} className={`design-option ${draft.designPreset===p.key?"selected":""}`} onClick={()=>selectDesign(p)}><div className="design-option-image" style={{backgroundImage:`linear-gradient(180deg,transparent 42%,rgba(5,13,12,.8)),url(${p.preview})`}}><span>추천 {i+1}</span><em>{p.tags?.join(" · ")}</em></div><div className="design-option-body"><div><b>{p.name}</b>{draft.designPreset===p.key&&<i>선택됨</i>}</div><p>{p.description}</p>{p.layers?<div className="layer-contract"><span>{p.layers.foundation}</span><span>{p.layers.molecule}</span><span>{p.layers.organism}</span></div>:<small>무드 참고 · {p.inspiredBy?.join(" / ")}</small>}<div className="palette-row">{Object.values(p.colors).slice(0,5).map((color,j)=><span key={j} style={{background:`#${color}`}}/>)}<mark>{p.fit}</mark></div></div></button>)}</div><label className="design-note">추가 디자인 요청<textarea value={draft.designRequest} onChange={e=>field("designRequest",e.target.value)} placeholder="예: 기관 로고 컬러는 유지하고 사진 비중을 높여주세요."/></label><p className="design-legal">첨부한 Atomic Design 원칙을 구조와 검증 기준으로 사용하며, 외부 브랜드 화면이나 장식 요소를 그대로 복제하지 않습니다.</p></>}
